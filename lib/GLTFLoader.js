...
THREE.GLTFLoader = ( function () {
      ...
      if ( json.extensionsUsed ) {
        ...
        for ( var i = 0; i < json.extensionsUsed.length; ++ i ) {
          ...
          switch ( extensionName ) {
            ...
            case EXTENSIONS.DRAFT_MATERIALS_PBR_NEXT:
              extensions[ extensionName ] = new GLTFMaterialsPbrNext( json );
              break;
  ...
  var EXTENSIONS = {
    ...
    DRAFT_MATERIALS_PBR_NEXT: 'DRAFT_materials_pbrNext',
  ...
  /**
   * PBR Next Extension (EXPERIMENTAL)
   */
  function GLTFMaterialsPbrNext() {

    this.name = EXTENSIONS.DRAFT_MATERIALS_PBR_NEXT;

  }

  GLTFMaterialsPbrNext.prototype.getMaterialType = function () {

    return THREE.MeshPhysicalMaterial;

  };

  GLTFMaterialsPbrNext.prototype.extendParams = function ( params, material, parser ) {

    var pbrNext = material.pbrMetallicRoughness.extensions[ this.name ];

    if ( pbrNext.clearcoat !== undefined ) {

      params.clearCoat = pbrNext.clearcoat;

    }

    if ( pbrNext.clearcoatRoughness !== undefined ) {

      params.clearCoatRoughness = pbrNext.clearcoatRoughness;
  ...
  GLTFParser.prototype.loadMaterial = function ( materialIndex ) {
    ...
    if ( materialExtensions[ EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS ] ) {
      ...
      if ( (metallicRoughness.extensions||{})[ EXTENSIONS.DRAFT_MATERIALS_PBR_NEXT ] ) {

        var pbrNextExtension = extensions[ EXTENSIONS.DRAFT_MATERIALS_PBR_NEXT ];
        materialType = pbrNextExtension.getMaterialType( materialDef );
        pending.push( pbrNextExtension.extendParams( materialParams, materialDef, parser ) );
